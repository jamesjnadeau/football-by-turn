...
/**
 * The one writer of {id, aim, cover, shade} orders, whoever computed them.
 * Cover orders go through cover.js's setCover, so the computer's man coverage
 * IS the human's cover order — shade included, a leverage offset only the
 * learned defense ever hands over; everything else becomes an ordinary
 * full-throttle plan pointed at the order's aim. clearCover runs on anyone
 * not covering — a stale assignment from last turn must not keep steering a
 * man this turn.
 */
export function applyOrders(state, orders) {
  for (const { id, aim, cover, shade } of orders) {
    if (cover) { setCover(state, id, cover, shade ?? 0); continue; }
    clearCover(state, id);
    if (!aim) continue;
