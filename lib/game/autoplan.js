...
/** "One man reads run -- he is playing downhill." / "Two men read pass --
 *  they are giving ground." The subject-verb-pronoun agreement a count of
 *  committed defenders needs, spelled out rather than numeralled to match
 *  the file's other sentences. */
function committedSentence(n, direction) {
  const who = n === 1 ? 'One man' : `${COUNT_WORDS[n] ?? n} men`;
  const verb = n === 1 ? 'reads' : 'read';
  const pronoun = n === 1 ? 'he' : 'they';
  const be = n === 1 ? 'is' : 'are';
  const doing = direction === 'run' ? 'playing downhill' : 'giving ground';
  return `${who} ${verb} ${direction} -- ${pronoun} ${be} ${doing}.`;
}
...
function defenseNote(state, genome) {
  ...
  // The read is per defender now (read.js), not one belief for the whole
  // unit -- so what there is to report is how many men have committed, and
  // which way. Nobody committed is not a call and is not reported: a defense
  // that has not made its mind up has nothing to tell the coach across the
  // table.
  const reads = Object.values(state.playRead?.reads ?? {});
  const run = reads.filter((r) => r.committed && r.pass < 0).length;
  const pass = reads.filter((r) => r.committed && r.pass > 0).length;
  if (!run && !pass) return scheme;
  if (run === pass) return `${scheme} The defense is split -- as many read run as pass.`;
  return run > pass
    ? `${scheme} ${committedSentence(run, 'run')}`
    : `${scheme} ${committedSentence(pass, 'pass')}`;
}
