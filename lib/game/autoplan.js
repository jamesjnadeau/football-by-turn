...
/**
 * Training mode's "computer plays both sides": draw up both halves of the
 * board at once, each with the same brain ai.js's coachAi would hand it —
 * autoplanLearnedOffense and autoplanLearnedDefense already wipe and redraw
 * one team apiece, so this is just both of them, in the order the board
 * reads (offense, then defense).
 */
export function autoplanBothTeams(state) {
  autoplanLearnedOffense(state);
  autoplanLearnedDefense(state);
  return 'The computer draws up both sides.';
}
 ...
 * definition the side the computer is not coaching. In a both-computer
 * training game there is no such side, so it draws up everyone instead —
 * overriding whatever was already on the board, on either team.
 */
export function autoplanLearned(state) {
  if (state.phase !== 'planning') return null;
  if (state.aiBoth) return autoplanBothTeams(state);
