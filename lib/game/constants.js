...
// short cosmetic tip the arrow itself draws to), not on a player's own body,
// so it earns a much fatter margin than the couple of units PICK_SLOP_UNITS
// gives every pick anchored to an actual body on the field. Widening
// PICK_SLOP_UNITS itself to fix this would have made every other pick in the
// game sloppier just to rescue one; this constant is scoped to the one target
// that needed it.
