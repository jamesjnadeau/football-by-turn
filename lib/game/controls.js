...
export function controlsFor(state, {
  ...
  add('autoplan', {
    label: state.aiBoth ? 'Autoplan both' : `Autoplan ${coachedSide(state)}`,
    disabled: !planning,
  });
  // In a match (another human's team is named by remoteTeam) the press does
  // not run anything here: it ends this coach's turn and waits for the other
  // coach's. The button says which it is.
  add('run', { label: state.remoteTeam ? 'End Turn' : 'Run Turn', disabled: !planning });
