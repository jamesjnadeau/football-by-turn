 ...
 *
 * `shade` is yards of downhill (or depth) leverage the order is played with —
 * see coverAim. Zero for an ordinary cover order; the learned defense is the
 * only caller who ever passes anything else.
 */
export function setCover(state, id, targetId, shade = 0) {
...
export function coverAim(state, player) {
  ...
  const shade = (point) => (player.coverShade
    ? { x: point.x, y: point.y + player.coverShade }
    : point);
  if (!car || car.team !== player.team || car.id === player.id) return shade(aim);
  const toBall = sub(car.pos, aim);
  if (len(toBall) === 0) return shade(aim);
  return shade(add(aim, scale(norm(toBall), t.radius + player.radius)));
}
