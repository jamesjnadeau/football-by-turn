...
export function setPersonnel(state, personnel) {
  ...
  // A cover order naming a man this package does not field is a lie for the
  // same reason a moved man's is: it was worked out from where somebody used
  // to stand (here, "somebody" means "on the field at all"), and that is no
  // longer true. Only the survivors can be checked against; a departed man's
  // own cover left with him.
  const onField = new Set(state.players.map((p) => p.id));
  for (const p of state.players) {
    if (p.cover !== null && !onField.has(p.cover)) p.cover = null;
  }
  return true;
}
