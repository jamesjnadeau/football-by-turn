...
export function setPersonnel(state, personnel) {
  ...
  // A blocker taken up on a man the swap just sent off is holding an order
  // pointed at nobody, and cover.js's coverAim looks that man up by id on
  // every physics sub-step — a dangling one is not a stale order, it is a
  // thrown `unknown player` in the middle of the turn. The arrow goes with
  // it, the rule placePlayer keeps: an aim worked out from where a man was
  // standing is a lie once he is not standing anywhere.
  for (const p of state.players) {
    if (!p.cover || state.players.some((q) => q.id === p.cover)) continue;
    p.cover = null;
    p.plan = null;
  }
  return true;
