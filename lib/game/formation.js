 ...
 *
 * Everything here is PURE except for the writers it names outright:
 * placePlayer, placeFormation, setPersonnel, applyLearnedLook and
 * answerOffense. Everything else reads `state` and returns facts or
 * positions; app/main.js is what decides when to ask.
 *
...
import {
  SIDELINE_LEFT, SIDELINE_RIGHT, CENTRE_X, hashCentresX, xToYards,
...
import { learnedDefenseSpots, MAX_YARD, isLearnedDefense } from './learned/formation.js';
...
export function setPersonnel(state, personnel) {
  ...
  // A blocker taken up on a man this package does not field is holding an
  // order pointed at nobody, and cover.js's coverAim looks that man up by id
  // on every physics sub-step — a dangling one is not a stale order, it is a
  // thrown `unknown player` in the middle of the turn. The arrow goes with it,
  // the rule placePlayer keeps: an aim worked out from where a man was
  // standing is a lie once he is not standing anywhere. Only the survivors can
  // be checked against; a departed man's own cover left with him.
 ...
 * Nickel and dime share the axis and its weights, differing only in where
 * their own bias sits. Nothing orders `sub:dime:bias` against
 * `sub:nickel:bias` — a genome is free to learn either one looser than the
 * other — and dime is tested first, so whichever cut is looser wins
 * regardless of which package's name it carries. A genome that learns a
 * looser dime cut than nickel plays dime on every look that would have drawn
 * nickel, and nickel becomes unreachable: exactly the shipped genome (dime
 * bias -1.65, nickel bias -2.65), which shows up as always-stacked rather
 * than as a bug only because this roster never drives the axis past either
 * cut.
 *
...
export function defenseKeys(state, team = 'defense') {
  ...
  // Both of today's callers (alignDefense, learnedLook) return early when
  // `them` is empty, so the ball.x fallback below never actually runs from
  // here — it is a guard for a caller that does not exist yet, not a case
  // either of these reaches.
 ...
 *
 * defenseKeys decides who each man answers; this turns that pairing into a
 * spot on the field:
 *
...
export function alignDefense(state, team = 'defense') {
  ...
  for (const d of mine) {
    ...
    if (group === 'line') {
      ...
      // what the coach lines up against is what he will be playing against. A
      // defense with one backer gets a lane of zero and lands on the ball,
      // which is exactly where the `other` branch below puts him — the two
      // only diverge once there is more than one backer to spread.
      aim.set(d.id, {
 ...
 */
export function learnedLook(state, values) {
  const g = clampGenome(DEFENSE_SPEC, values);
  ...
  for (const d of state.players) {
    ...
    // drawn field deep in the red zone. learnedDefenseSpots has no 0.5 floor
    // of its own, so the two could in principle disagree — but only when
    // MAX_YARD - losYard drops below 0.5, and rules.js never lets losYard
    // reach a yard that close to MAX_YARD (it clamps to at most
    // GOAL_YARD - 0.5, and GOAL_YARD sits well short of MAX_YARD). The floor
    // is real; it just never binds in a game this rulebook can deal. That is
    // what lets "zero pull equals learnedDefenseSpots" stand as a guarantee
    // rather than a usual case.
