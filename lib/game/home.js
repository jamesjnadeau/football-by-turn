...
/**
 * The sides a MATCH offers, which are not the sides a game does. Two, because
 * a drive has two of them and Training Mode is a thing you do alone — and
 * worded for the opponent a coach is actually about to get. The single-player
 * notes promise him the computer's learned defense; in a match the defense is
 * a person, and saying otherwise is the screen lying about the whole point of
 * the feature.
 */
export const MULTIPLAYER_SIDES = [
  {
    id: 'offense',
    label: 'Play Offense',
    note: 'You call the runs and throws. Another coach is setting the defense against you.',
  },
  {
    id: 'defense',
    label: 'Play Defense',
    note: 'You set the coverage. Another coach is calling the plays.',
  },
];
...
/**
 * What a press on the home section means — the decision, lifted out of the
 * listener so it can be tested, because the listener itself cannot be.
 *
 * `owns` is the whole reason this is a function rather than four `if`s in
 * app/home.js. The section is not the home screen's alone: the multiplayer
 * lobby draws its own side chooser into it, out of this same `sideMarkup`,
 * so those buttons carry the same `data-side` the home screen matches on.
 * A listener that stays attached to a section it has handed over will act on
 * the new screen's buttons as if they were its own — which is how one press
 * on the lobby's chooser also started a single-player game against the
 * computer, and left whichever game finished starting last on the board.
 *
 * Ownership is the fix rather than a different attribute for the lobby's
 * buttons, because the next screen drawn into this section would have the
 * same problem and no reason to know it.
 */
export function homeAction({ tutorial, back, side, variant } = {}, { owns = true } = {}) {
  if (!owns) return null;
  if (tutorial) return { kind: 'tutorial' };
