...
export function humanSide(state) {
  ...
  // In a match the other team is another human's (state.remoteTeam, set by
  // app/main.js), and this coach's side is the one that is not his. Without
  // this both coaches read as hot-seat, and the defense coach's Autoplan,
  // playbook and coaching log all act on the offense.
  if (state.remoteTeam === 'offense') return 'defense';
  if (state.remoteTeam === 'defense') return 'offense';
  return null;
}
