 ...
 * pushing the commit threshold out of the evidence's reach rather than by
 * using it.
 *
 * There is no bare-aim half any more: with the read moved onto the man he is
 * covering, only a man who HAS a cover order has a read to act on in the
 * first place — the front, the deep free man and every zone defender have
 * nobody to key off and so are never in `reads` at all. A man who has been
 * given somebody to cover keeps him, whichever way his own read went; what
 * moves is HOW he plays that man: a shade — read:trigger yards of downhill
 * (or depth) leverage on cover.js's aim, applied every sub-step the order
 * stands, toward the line on a committed run and away from it on a committed
 * pass. A man who bites on a fake is LATE to his man — not absent from the
 * play.
 */
function applyTrigger(state, team, p, order, reads, genome) {
  if (!order.cover) return order; // no man, no read, no leverage
  ...
  return {
    ...order,
    shade: genome['read:trigger'] * read.confidence * UNITS_PER_YARD_X * dir * toward,
  };
}
...
export function learnedOrders(state, team, genome, tendencies = null) {
  ...
  orders.push(...zone);
  return orders.map((o) => applyTrigger(state, team, getPlayer(state, o.id), o, p.reads, genome));
}
