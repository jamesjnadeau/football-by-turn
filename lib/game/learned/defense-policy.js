 ...
 * state (plus a genome) and returns {id, aim, cover, shade} orders; ai.js is
 * the only writer. The structure is hand-written and the numbers are learned:
 *
 ...
 *
 * A symmetric slide, not a coverage drop. The first build took a committed
 * RUN read as license to abandon a man entirely and point at the carrier —
 * catastrophic when the read was wrong (a receiver run completely free) for
 * little gain when it was right (guard three already converges the whole
 * defense on the carrier once he is past the line), and training answered by
 * pushing read:commit out of the evidence's reach rather than by using it.
 *
 * A man who has been given somebody to cover keeps him, whichever way the
 * read went. What moves for him is HOW he plays that man: a cover order gets
 * a shade — read:trigger yards of downhill (or depth) leverage on cover.js's
 * aim, applied every sub-step the order stands. In man coverage that shade is
 * the whole of the trigger, because every triggerable man IS covering
 * somebody; a version that only slid bare aims would reach nobody in man and
 * the genome would train to always-man. Everybody else's AIM slides instead,
 * the same read:trigger yards at the read's own confidence. Either way the
 * direction is toward the line of scrimmage on a committed run and away from
 * it on a committed pass. A man who bites on a fake is LATE to his man, his
 * zone, or his flow angle — not absent from the play.
 */
function applyTrigger(state, team, p, order, read, genome) {
  if (!read.committed || !triggerable(state, team, p)) return order;
  const dir = defendDir(team);
  // Toward the line on a committed run read, away from it on a committed pass.
  const toward = read.pass < 0 ? -1 : 1;
  const give = genome['read:trigger'] * read.confidence * UNITS_PER_YARD_X * dir * toward;
  if (order.cover) return { ...order, shade: give };
  if (!order.aim) return order;
  return { ...order, aim: { x: order.aim.x, y: order.aim.y + give } };
}
