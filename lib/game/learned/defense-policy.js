 ...
 * read went — a cover order (`order.aim === null`) is untouched by either
 * branch below. What moves is everybody else's AIM: read:trigger yards of it,
 * at the read's own confidence, toward the line of scrimmage on a committed
 * run and away from it on a committed pass. A backer who bites on a fake is
 * LATE to his zone or his flow angle, not absent from the play; a defender
 * who reads pass correctly gives ground exactly where the old code already
 * sent him.
 */
function applyTrigger(state, team, p, order, read, genome) {
  ...
  return { ...order, aim: { x: order.aim.x, y: order.aim.y + dir * toward * give } };
}
