...
F.push(
  ...
  // What each covering defender makes of the man he has been given
  // (lib/game/read.js). A single team-wide read was measured at 28.1%
  // correct and scrapped; the read is per defender now, keyed off his own
  // man, in logit units positive for pass:
  //
  //   z_d(t) = bias + inertia*z_d(t-1) + downfield*cue.downfield + lateral*cue.lateral
  //
  // Every weight is zero at init, read:man:commit included, so an untrained
  ...
  //
  // read:man:bias is not decoration. downfield's cue and lateral's cue are
  // both non-negative by construction — lateral is an absolute value, and
  // downfield is positive on nearly every turn — so with no intercept term
  // the SIGN of z_d is fixed by the weights alone: the model can answer only
  // one class for every input it will ever see, and six training runs all
  // landed on "run", always. Init 0 keeps z_d identically 0 at the untrained
  // genome (0 + 0 + 0 + 0), so the byte-for-byte guarantee still holds; what
  // the bias buys is a decision boundary that selection can slide off the
  // origin, onto whatever non-zero cue value the two classes actually part on.
  { key: 'read:man:bias', min: -4, max: 4, init: 0 },
  { key: 'read:man:downfield', min: -4, max: 4, init: 0 },
  { key: 'read:man:lateral', min: -4, max: 4, init: 0 },
  ...
  { key: 'read:man:inertia', min: 0, max: 1, init: 0 },
  ...
  // The ceiling is 4, not 8: a threshold no attainable evidence can cross is
  // the trap this design fell into twice, and a tight range is the cheapest
  // guard against it.
  { key: 'read:man:commit', min: 0, max: 4, init: 0 },
  // Yards of leverage a covering man plays with once his own read commits --
  // downhill on a run read, deeper on a pass read -- at full confidence.
