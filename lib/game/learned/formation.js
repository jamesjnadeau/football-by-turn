 ...
 * maybeApplyLearnedFormations (below) and realignLearnedDefense (app/main.js's
 * mid-down hook) share one gate instead of two copies of the same three
 * comparisons — the module comment's "two rulebooks cannot drift apart
 * silently" promise, applied to this condition as much as to the spots
 * themselves.
 *
 * Checked against baseVariantId, not variantId itself: nickel and dime are
 * personnel packages within the seven-a-side game, not different games, so a
 * genome trained for '7' is trained for '7-nickel' and '7-dime' too. The
 * distinction this gate exists to make is seven-a-side against eleven, not
 * stacked against a substitution.
 */
...
/**
 * app/main.js's realignDefense() calls this before it ever reaches the old
 * rule-based alignDefense. A learned defense's pre-snap look is a fixed spot
 * per genome (learnedDefenseSpots does not read the offense at all beyond
 * collision-avoidance nudging) — it does not "answer" a changed offensive
 * look, it just holds its ground. But alignDefense knows nothing about the
 * genome and will happily stomp it back to the rule-based look the instant
 * the human drags a player, calls a play, or changes personnel. Reapplying
 * the genome here — same gate as maybeApplyLearnedFormations, same write —
 * is what keeps a learned defense looking learned all the way through
 * planning, not just for the first frame of the down.
 *
 * Returns false (and touches nothing) when the learned-defense condition
 * does not hold, so the caller knows to fall back to alignDefense.
 */
