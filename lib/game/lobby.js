...
/**
 * What a coach sees while the page is getting his match back -- after a
 * reload, or on a connection that dropped before the board was ever drawn.
 * Give up goes home; the server lets the seat go on its own if nobody sits
 * back down in time.
 */
export function rejoinMarkup({ variant, side }) {
  return `<h1>${escapeText(variant.label)}</h1>`
    + `<p class="home-blurb">Rejoining your match as the ${escapeText(side)}…</p>`
    + '<div class="home-choices">'
    + '<button class="home-choice" type="button" data-lobby-back>'
    + '<span class="home-choice-label">Give up</span>'
    + '<span class="home-choice-note">Leave the match and go home.</span>'
    + '</button></div>';
}
