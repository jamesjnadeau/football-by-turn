...
/**
 * The landing circle currently shown on the board, if a lob's is — the same
 * one passLandingMark/liveLobMark draw, whether it is still being aimed this
 * turn (state.plannedPass, held by the passer, not locked onto a man) or
 * already thrown and still hanging in the air from an earlier one
 * (state.ball.lob). Checked in that order because releasePass leaves
 * plannedPass sitting there stale once the ball is gone from the passer's
 * hands; ball.lob is the one that is actually still true. Null whenever
 * neither is showing a circle — nothing thrown, a short throw, or a throw
 * locked onto a man instead of scattered.
 */
function currentLobLanding(state) {
  const lob = state.ball.lob;
  if (lob) return { pos: lob.aim, radius: lob.radius };
  const pp = state.plannedPass;
  if (!pp || pp.target || state.ball.carrierId !== pp.from) return null;
  return passLanding(getPlayer(state, pp.from), pp.dir, pp.power);
}

/**
 * The center of the currently shown lob landing circle, if `point` falls
 * inside it — the same forgiving PICK_SLOP_UNITS margin every other lock
 * gesture on the board gets, so a run drag that ends near the circle's edge
 * still counts as heading for the ball rather than an ordinary run past it.
 * Null when no circle is showing, or `point` misses the one that is.
 */
export function lobLandingAt(state, point) {
  const land = currentLobLanding(state);
  if (!land) return null;
  return dist(point, land.pos) <= land.radius + PICK_SLOP_UNITS ? land.pos : null;
}

/**
 * The loft a drag on the loft handle is asking for: how far back toward the
 * passer, ALONG THE THROW'S OWN LINE, the pointer has travelled since it
 * grabbed on. The handle sits on the near edge of the dead zone (render.js's
 * loftHandlePoint), and dragging it toward the passer is what pulls that
 * edge toward him — raising loft toward 1, more hang time, see lob.js's
 * lobSubsteps. Pushing it back downfield lowers loft toward 0, where it
 * floors — there is no equivalent of backOnPasser's "call the whole throw
 * off" for this smaller gesture. Read fresh from THIS grab's own
 * displacement rather than added to whatever loft was already set, so a
 * short, deliberate re-drag always means exactly what it visually shows.
 */
 ...
 *
 * Because the count comes from this undisturbed aim distance rather than the
 * scattered landing spot the real throw eventually rolls, a throw sitting
 * exactly on a turn boundary can occasionally resolve one turn later than its
 * last shown shadow ball promised — scatter nudges the real flight past the
 * boundary the preview never crossed. Accepted, not a bug: the alternative is
 * a preview that touches the random stream, which is worse.
 */
