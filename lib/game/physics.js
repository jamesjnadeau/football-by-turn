...
/**
 * Who is leaning on whom on the board as it STANDS, with nothing moved and
 * nothing pushed apart -- the same overlap test resolveCollisions makes, and
 * none of the impulse work it does afterwards.
 *
 * It exists so the board can show friction while the coach is still drawing.
 * The arcs were only ever painted from a turn's own frames, so two men already
 * hand-fighting went unmarked for the whole huddle and lit up the instant the
 * turn ran -- which is the one moment the coach can no longer do anything
 * about it.
 *
 * No `mu` on these contacts. The coefficient is a function of how fast the two
 * are rubbing past each other, and on a board at rest that question has no
 * answer worth drawing; the renderer only ever wanted the pair and the bearing
 * between them.
 */
export function contactsAt(state) {
  const contacts = [];
  ...
  for (let i = 0; i < players.length; i++) {
    for (let j = i + 1; j < players.length; j++) {
      ...
      const d = len(delta);
      // The same contact distance, arm's length and all: a blocker who has
      // taken a man on is engaged with him before their bodies actually meet,
      // and that is exactly the friction a coach wants warning of.
      const overlap = a.radius + b.radius + grabBonus(a, b) + driveReachBonus(a, b) - d;
      if (overlap <= 0) continue;
      const n = d === 0 ? { x: 0, y: 1 } : scale(delta, 1 / d);
      contacts.push({ a, b, normal: n });
    }
  }
  return contacts;
}
