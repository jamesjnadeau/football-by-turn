 ...
 *
 * Version 2 added `spots` — a play now carries the formation it was called
 * from, not just its arrows. Version 1 survives reading: a coach's five plays
 * outlive a format change, and a version-1 play is not a corrupt play, it is
 * a play with no formation in it, which sanitizePlay already knows how to
 * read as `spots: {}`. Writing is always the current version.
 */
...
export function parsePlaybook(text) {
  ...
  if (!READABLE.has(raw.v) || !Array.isArray(raw.slots)) return out;
