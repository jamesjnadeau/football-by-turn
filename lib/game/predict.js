...
/**
 * Every spot this player's OWN orders put him, one per sub-step of the coming
 * turn — the arrow he is actually carrying, or none at all, which is a man
 * coasting to a stop rather than one standing still.
 *
 * predictDestination answers "what if he were sent THIS way"; this answers
 * "where is he going to be, and when". A throw locked onto a man needs the
 * second question in full: aimed at where he stands, the ball arrives after he
 * has left, and aimed at where he finishes, a ball that cannot be thrown gently
 * enough crosses that spot before he reaches it.
 */
export function predictRoute(player) {
  const g = {
    ...player,
    pos: { ...player.pos },
    vel: { ...player.vel },
...
/** Where that route ends: his spot at the whistle. */
export function predictOwnDestination(player) {
  const route = predictRoute(player);
