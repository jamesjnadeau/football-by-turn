 ...
 *
 * Idempotent for the whole down, off that same `snapped` flag: the training
 * harness advances the percept itself before it gives orders (see
 * train/harness.js's defenseCoach), and runTurn calls this again a moment
 * later on the same turn. The first call sets `snapped` and freezes `look`;
 * every call after that, this turn or any later one in the down, finds
 * `snapped` already true and touches nothing.
 */
