 ...
 *
 * There used to be a third cue, ballAir (nobody is carrying it). It is gone:
 * learnedOrders returns early on `!carrier`, which is exactly the condition
 * that made the cue read `1`, so it could never actually inform an action.
 * Worse, being a pure indicator its weight could only ever push the
 * accumulator toward pass, which under one shared threshold left the run side
 * with less reach than the pass side for no footballing reason at all.
 */
