...
export const STYLE_GAME = [
  ...
  '.fbtn-plate{fill:#fff;fill-opacity:.92;stroke:#1a7f37;stroke-width:.8;pointer-events:all;cursor:pointer}',
  '.fbtn-icon{font:11px system-ui,sans-serif;text-anchor:middle;dominant-baseline:central;pointer-events:none}',
  ...
  '.fbtn-on .fbtn-plate{fill:#1a7f37;fill-opacity:.25;stroke-width:1.6}',
...
function fieldButtonMark({ attr, icon, label, cy, on = false, off = false, pressed = null }) {
  ...
  return (
    ...
    ` x="${num(PRESS_BOX_X - FIELD_BTN_SIZE / 2)}" y="${num(cy - FIELD_BTN_SIZE / 2)}"` +
    ` width="${num(FIELD_BTN_SIZE)}" height="${num(FIELD_BTN_SIZE)}" rx="3"/>` +
    `<text class="fbtn-icon" x="${num(PRESS_BOX_X)}" y="${num(cy)}">${escapeText(icon)}</text>` +
    `</g>`
  );
}
