...
// A play is what you come to the line with, so the playbook column's greying
// asks the same question the Coaches Menu's Plays section does: canUsePlays
// for when saving and calling are legal, PLAY_SLOTS for how many plates the
// five-slot column needs.
...
/**
 * The band of quick-press plates down the right-hand margin: two columns of
 * them, with the Coaches Menu holding the middle of the first, where the
 * sideline legend used to be spelled out. They are for what a coach does over
 * and over — shuffle the formation, run the turn, sweep the arrows away, call
 * a play off the book — so they sit on the board rather than behind something
 * that must be opened first. FIELD_BTN_GAP is the clear space between plates.
 */
...
export function cameraViewBox(losYard, cameraYard) {
  ...
  return `0 ${num(view.windowTopY)} ${GAME_VIEWBOX_WIDTH} ${num(view.height)}`;
}
...
export function renderBoardShell(losYard, toGoYard, cameraYard = losYard, { menu = true } = {}) {
  ...
  return {
    viewBox: `0 ${num(view.windowTopY)} ${GAME_VIEWBOX_WIDTH} ${num(height)}`,
    markup:
...
/**
 * The column, as a table. `col` is the plate's distance from the first column
 * in the same pitch, so the band is a square grid: column 0 is the one that
 * has always been here, and column 1 is the playbook out in the air the wider
 * crop opened up. `slot` is the plate's distance from the middle in
...
const FIELD_BUTTONS = {
  ...
  play1: { attr: 'data-play-button', attrValue: '0', icon: '1\u{FE0F}\u{20E3}', col: 1, slot: -2 },
  play2: { attr: 'data-play-button', attrValue: '1', icon: '2\u{FE0F}\u{20E3}', col: 1, slot: -1 },
  play3: { attr: 'data-play-button', attrValue: '2', icon: '3\u{FE0F}\u{20E3}', col: 1, slot: 0 },
  play4: { attr: 'data-play-button', attrValue: '3', icon: '4\u{FE0F}\u{20E3}', col: 1, slot: 1 },
  play5: { attr: 'data-play-button', attrValue: '4', icon: '5\u{FE0F}\u{20E3}', col: 1, slot: 2 },
};

/**
 * The playbook column is drawn by counting to PLAY_SLOTS and looking each slot
 * up by name, so a sixth slot added to playbook.js without a sixth row here
 * would ask for a plate that does not exist and take the whole board down with
 * a TypeError halfway through a paint. Checked once at load instead, so the
 * failure says which of the two moved.
 */
const PLAY_PLATES = Object.keys(FIELD_BUTTONS).filter((name) => name.startsWith('play'));
if (PLAY_PLATES.length !== PLAY_SLOTS) {
  throw new Error(
    `FIELD_BUTTONS holds ${PLAY_PLATES.length} play plates, but the playbook has ${PLAY_SLOTS} slots`,
  );
}
...
/** Every control the band knows about. A test seam: the suite walks this to
 *  check that every row of the table is actually drawn somewhere, and nothing
 ...
 * without these a keyboard would have no way to reach any of these plates at
 * all.
 *
...
/**
 * Every plate but the clipboard, drawn into `game-buttons` by app/main.js on
 * every paint rather than built once with the board — which is what lets them
 * change with the game. A rule that says no greys its plate, the shuffle goes
 * altogether, and a play slot's label follows whatever is saved in it; none of
 * that could be settled at the snap. The menu hit rect beside them never
 * changes, so it stays in the shell.
 *
 * The shuffle is the one plate that vanishes rather than greying:
 * repositioning is illegal once the first turn has run, so rather than grey
 * the button out it goes, which is also the coach's cue that the play is under
 * way. Everything else is the opposite — always in its place, greyed while it
 * cannot be pressed, because a button that moves or vanishes is one you have
 * to look for.
 *
 * `book` and `aiLabel` are handed in rather than read: the play library lives
 * in app/main.js behind localStorage and the mode label lives in ai.js behind
 * the whole computer coach, and this stays a presentation module that is a
...
export function renderFieldButtons(
  ...
  // teaching. A normal drive passes nothing and gets the band it always had.
  // The menu is deliberately not one of the plates below: it is drawn by
  ...
  const parts = [];
  /**
   * One plate, off its row of the table. What every plate shares — where it
   * sits, what it is marked with, which attribute a press is matched on — is
   * read from FIELD_BUTTONS, so a control is named once here instead of four
   * times; what varies is passed in at the call, so each plate's label and
   * each plate's rule are still written out where a reader can see them.
   */
  const plate = (name, opts) => {
    if (!fielded(name)) return;
    const b = FIELD_BUTTONS[name];
    const at = fieldButtonAnchor(name, los, cam);
    parts.push(fieldButtonMark({
      attr: b.attr, attrValue: b.attrValue, icon: b.icon, cx: at.x, cy: at.y, ...opts,
    }));
  };

  // The one plate drawn behind a condition rather than given an `off` rule:
  // this is the one that goes rather than greys.
