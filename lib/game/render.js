...
import { getPlayer, SNAP_TARGET_ID } from './state.js';
 ...
 */
export function renderFieldButtons(state, { repositioning = false, animating = false } = {}) {
  // One pitch either side of the menu plate, which holds the middle.
  const midY = buttonColumnMidY(state.losYard);
  const offset = FIELD_BTN_PITCH;
  const parts = [];
  if (canReposition(state) && !animating) {
    parts.push(fieldButtonMark({
      attr: 'data-reposition-button',
      icon: '\u{1F500}',
 ...
 */
export function renderMessage(text, losYard) {
  const lines = wrapWords(text, MESSAGE_MAX_CHARS);
  if (lines.length === 0) return '';

  const view = gameView(losYard);
  // Centred in the end zone when the current window actually reaches it —
  // the same "goalYard is null until the camera gets there" test render.js's
  // own lineToGainMark uses. Once the camera has scrolled well downfield the
  // end zone is off-window, so the plate falls back to the middle of
  // whatever window IS actually drawn, the same spot menuButtonMark/
  // buttonColumnMidY use — anything tied to the fixed GOAL_YARD/END_YARD
  // yards regardless of the window would draw off-screen the moment the
  // camera scrolls away from them (design decision 9's bug, generalized to
  // this file's own third offender).
  const zoneTop = view.goalYard != null ? yardToY(view, view.goalYard) : view.fieldTopY;
  const zoneBottom = yardToY(view, view.bottomYard);
  const midY = (zoneTop + zoneBottom) / 2;
  ...
  // Centred in the end zone (or the window's middle, above), but clamped to
  // the currently drawn window — view.windowTopY/height, not the fixed 0 the
  // single-window game used, so the plate always lands somewhere inside the
  // viewBox actually being shown rather than the window some other losYard
  // would have framed. A three-line message — the illegal-pass penalty is
  // one — is taller than the end zone, so it grows up over the goal line
  // rather than sliding off the bottom edge. Messages are not shortened to
  // fit: the renderer is the thing that has to cope.
  //
  // The clamp holds for any message that fits the window at all — up to
  // fourteen lines. Past that the plate is taller than the window itself, so
  // it pins to the top and overflows the bottom; that would be a caller
  // writing an essay into a status line, and truncating it silently would
  // hide the bug rather than show it.
  const plateY = Math.max(
    view.windowTopY + MESSAGE_PAD / 2,
    Math.min(midY - plateH / 2, view.windowTopY + view.height - plateH - MESSAGE_PAD / 2),
  );
...
/**
 * The planned throw, in one of its four shapes: the snap, locked onto a man,
 * arcing to a landing circle, or the plain arrow it has always been. Nothing is
...
export function renderPassArrow(state) {
  ...
  // A drawn throw's arrow is as long as it is hard, which is the coach's own
  // drag played back to him. The snap has no such length to show: its power is
  // solved from a gap of a few yards and comes out clamped at the minimum, so
  // scaling by it would draw an arrow of exactly nothing. It is pointed at the
  // man it is going to instead -- which is the useful thing to see about an
  // order nobody drew. It is nobody's lob and nobody's lock-on either, so it
  // is answered first and on its own.
  const snapTo = planned.auto && state.players.find((p) => p.id === SNAP_TARGET_ID);
  if (snapTo) {
    return `<g class="plan-arrow" data-pass="${planned.from}">${passArrowMark(from.pos, snapTo.pos)}</g>`;
  }
