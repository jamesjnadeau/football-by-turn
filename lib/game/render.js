 ...
 */
import { VIEWBOX_WIDTH, YARD_LABEL_LEFT_X, CENTRE_X, SIDELINE_LEFT, SIDELINE_RIGHT, UNITS_PER_YARD_X, y as yardToY } from '../field/geometry.js';
...
import { unplannedPlayers, frictionsAt } from './turn.js';
...
import {
  ...
  FRICTION_ARC_DEGREES,
  LOFT_HANDLE_RADIUS_UNITS,
} from './constants.js';

/**
 * The yard numbers measure a shade over ten units wide at their size
 * (browser-measured — SVG text has no width until it is laid out). The play
 * clock is centred in the margin between the frame's left edge and them.
 */
...
export const STYLE_GAME = [
  ...
  // The friction arc: orange, because every other cue on this board is already
  // spoken for — green is a cover order, red a throw, gold the tutorial's
  // ring. Unlike those it is drawn ABOVE the players, since it is a report on
  // what is happening to a man this instant rather than a standing order under
  // him, and a rim hidden beneath him would be no report at all. No pulse:
  // there may be a dozen of these on the field at once.
  '.friction-arc{fill:none;stroke:#e8710a;stroke-width:1.2;stroke-linecap:round;pointer-events:none}',
  ...
  // The loft handle: a short bar laid across the flight line so the spot the
  // coach can grab to raise or lower the arc — otherwise just empty space
  // since the bold power arrow stopped drawing to it — reads as a handle.
  '.pass-loft-handle{stroke:#b3261e;stroke-width:1.5;stroke-linecap:round;opacity:.9;pointer-events:none}',
  ...
  // The ring around a player still missing an arrow. Same gold as the
  // tutorial's, so it reads as the same kind of cue, but a full flash rather
  // than a pulse -- and slower -- so it stands out as "still needs you" among
  // a crowd of players rather than blending in as one more animated marking.
  '.unplanned-ring{fill:none;stroke:#ffd23f;stroke-width:1;pointer-events:none;animation:unplanned-flash 2.4s ease-in-out infinite}',
  '@keyframes unplanned-flash{0%,49%{opacity:1}50%,100%{opacity:0}}',
].join('');
...
export function renderBoardShell(losYard, toGoYard, cameraYard = losYard) {
  ...
  return {
    ...
    markup:
      ...
      // players; friction arcs go over them, because a rim drawn on a man is a
      // report about him and hiding it beneath him would say nothing; the
      // overlay stays on top of both, because that is where the animated loose
      // ball is drawn.
      `<g id="game-arrows"></g><g id="game-preview"></g>` +
      `<g id="game-players"></g><g id="game-friction"></g><g id="game-overlay"></g>` +
      `<g id="game-message"></g>` +
...
/**
 * One point of friction, drawn on one man: an arc of FRICTION_ARC_DEGREES
 * centred on `angle`, the bearing from him toward the opponent leaning on him.
 * Swept just outside his own circle, at the same standoff the cover halo uses,
 * so it reads as a rim on him rather than a ring around him.
 *
 * Drawn in absolute field coordinates rather than a player group's local
 * space: these are painted per frame from the turn's own snapshots, which know
 * where a man is but have no group to hang anything inside.
 */
export function frictionArcMark(pos, radius, angle) {
  const half = (FRICTION_ARC_DEGREES / 2) * (Math.PI / 180);
  const r = radius + COVER_HALO_UNITS;
  const at = (a) => ({ x: pos.x + r * Math.cos(a), y: pos.y + r * Math.sin(a) });
  const p0 = at(angle - half);
  const p1 = at(angle + half);
  // large-arc 0, sweep 1: the short way round, in the direction of increasing
  // angle -- which is the slice between the two ends and not the rest of the
  // circle. FRICTION_ARC_DEGREES stays under 180 so the short way is the
  // right way.
  return `<path class="friction-arc" d="M ${num(p0.x)} ${num(p0.y)}`
    + ` A ${num(r)} ${num(r)} 0 0 1 ${num(p1.x)} ${num(p1.y)}"/>`;
}

/**
 * Every friction arc in one frame: the turn's snapshot says which man is being
 * leaned on and from which bearing, this looks up where he is in that same
 * frame and how big he is, and draws it.
 */
export function frictionArcsMark(frame, radiusOf) {
  const at = new Map(frame.players.map((p) => [p.id, p]));
  return (frame.frictions || []).map((f) => {
    const p = at.get(f.id);
    const r = radiusOf(f.id);
    return p && r ? frictionArcMark(p, r, f.angle) : '';
  }).join('');
}

/**
 * Every friction arc for the board as it STANDS, rather than for a frame of a
 * turn being animated. Same arcs, same rule, read off the players themselves
 * instead of a snapshot -- so a coach drawing next turn's orders can see which
 * of his men are already engaged, while he can still do something about it.
 */
export function frictionArcsAtMark(state) {
  const at = new Map(state.players.map((p) => [p.id, p]));
  return frictionsAt(state).map((f) => {
    const p = at.get(f.id);
    return p ? frictionArcMark(p.pos, p.radius, f.angle) : '';
  }).join('');
}
...
/**
 * A flashing ring around every player still missing an arrow -- the same cue
 * pressRun's own warning gives in text, but visible on the field before the
 * coach ever presses the button.
 */
export function unplannedRingsMark(state) {
  return unplannedPlayers(state)
    .map((id) => getPlayer(state, id))
    .map((p) => `<circle class="unplanned-ring" cx="${num(p.pos.x)}" cy="${num(p.pos.y)}" r="${num(p.radius + 2)}"/>`)
    .join('');
}
...
/**
 * The thin dotted line passLockMark draws to an auto-aimed target — a locked-on
 * drag or the automatic snap. The thick `.pass` arrow reads as "this is the
 * force the coach put behind it", which is the wrong cue here: a lock-on's
 * power is solved at the whistle (lockOnPass), not drawn from the drag, so the
 * bold arrow was overstating a throw the coach never actually loaded up. The
 * same `.pass-flight` weight the flight-path preview already uses says
 * "this is where it's going" instead, with the arrowhead kept so the
 * direction still reads at a glance.
 */
export function passLockLineMark(from, to) {
  return `<path d="M ${num(from.x)} ${num(from.y)} L ${num(to.x)} ${num(to.y)}" class="pass-flight" marker-end="url(#ar-r)"/>`;
}
 ...
 */
export function passFlightMark(from, to, loft = 0) {
  const total = dist(from, to);
  const { start, end } = deadZoneSpan(total, loft);
  const at = (d) => ({
...
/**
 * Where the loft handle sits: the near edge of the dead zone — the boundary,
 * at this throw's current loft, between "still out of the hand, still
 * catchable" and "up over everyone". Not a cosmetic point off on its own;
 * it is deadZoneSpan's own `start`, walked out along the same hand-to-aim
 * line passFlightMark draws, so the handle always sits exactly on the
 * boundary the flight path is itself drawing.
 */
export function loftHandlePoint(from, to, loft) {
  const total = dist(from, to);
  const { start } = deadZoneSpan(total, loft);
  return {
    x: from.x + ((to.x - from.x) * start) / total,
    y: from.y + ((to.y - from.y) * start) / total,
  };
}

/**
 * The bar the coach grabs to raise or lower a lob's loft: a short stroke laid
 * across the throw's own line, centred on loftHandlePoint — the near edge of
 * the dead zone itself, not a bare cosmetic tip off on its own. Dragging this
 * bar toward the passer pulls that boundary toward him by exactly the same
 * amount (see loftFromDrag in pass.js), so growing the "nobody can catch it"
 * stretch is not a side effect of the drag — the drag IS the boundary moving.
 * Perpendicular to the throw, not along it, so it reads as a handle rather
 * than a second arrowhead. Drawn at half of LOFT_HANDLE_RADIUS_UNITS —
 * narrower than the grabbable area itself, the way a touch target is usually
 * bigger than the control drawn inside it — so the mark reads as a small
 * handle rather than a barrier across the throw.
 */
export function passLoftHandleMark(from, to, loft) {
  const half = LOFT_HANDLE_RADIUS_UNITS / 2;
  const total = dist(from, to);
  const p = loftHandlePoint(from, to, loft);
  const dir = { x: (to.x - from.x) / total, y: (to.y - from.y) / total };
  const perp = { x: -dir.y, y: dir.x };
  const a = { x: p.x + perp.x * half, y: p.y + perp.y * half };
  const b = { x: p.x - perp.x * half, y: p.y - perp.y * half };
  return `<path d="M ${num(a.x)} ${num(a.y)} L ${num(b.x)} ${num(b.y)}" class="pass-loft-handle"/>`;
}
...
export function passLockMark(passer, receiver) {
  ...
  return (
    ...
    passLockLineMark(passer.pos, tip)
  );
}
  ...
  if (locked) {
    ...
    mark = land
      ? passLandingMark(land.pos, land.radius)
        + passFlightMark(passOrigin(from, planned.dir), passAim(from, planned.dir, planned.power), planned.loft ?? 0)
...
/**
 * The loft handle, on its own rather than folded into renderPassArrow's own
 * marks — app/main.js layers this one into `game-overlay`, the topmost
 * layer, instead of `game-arrows` underneath the players. A handle a
 * receiver's own body happened to stand on top of would be a handle the
 * coach could no longer see to grab; the rest of the throw preview has no
 * such requirement and stays where it was.
 */
export function renderLoftHandle(state) {
  const planned = state.plannedPass;
  if (!planned || planned.target || state.ball.carrierId !== planned.from) return '';
  const from = getPlayer(state, planned.from);
  if (!passLanding(from, planned.dir, planned.power)) return '';
  const origin = passOrigin(from, planned.dir);
  const aim = passAim(from, planned.dir, planned.power);
  return (
    `<g class="plan-arrow" data-pass="${planned.from}">`
    + passLoftHandleMark(origin, aim, planned.loft ?? 0)
    + '</g>'
  );
}
