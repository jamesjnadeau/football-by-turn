 ...
 * an equilateral triangle centred on his centre, apex pointing along his
 * velocity, out to (radius + speed × DEBUG_VELOCITY_SECONDS) ×
 * DEBUG_VELOCITY_TRIANGLE_SCALE — the same reach the old hairline drew,
 * scaled down so a filled shape doesn't swallow the player it marks. The
 * radius term is what puts the apex outside his own circle at any speed that
 * rounds to a visible difference; the length BEYOND his edge is the part
 * that reads as velocity, and it is strictly proportional to it. The other
 * two vertices trail the apex by ±120°, at the same distance from the
 * centre, which is what makes the circumcentre — and so the triangle's
 * visual centre — sit exactly at the origin regardless of heading. The
 * `speed === 0` guard exists only to dodge dividing by zero when computing
 * the heading (which would draw NaN), not to catch "slow": standing still
 * there is no direction to point, so a stopped player gets nothing.
 */
function velocityTriangle(player) {
  const { x, y } = player.vel;
  const speed = Math.hypot(x, y);
  if (speed === 0) return '';
  const r = (player.radius + speed * DEBUG_VELOCITY_SECONDS) * DEBUG_VELOCITY_TRIANGLE_SCALE;
  const theta = Math.atan2(y, x);
  const points = [theta, theta + (2 * Math.PI) / 3, theta + (4 * Math.PI) / 3]
    .map((a) => `${num(r * Math.cos(a))},${num(r * Math.sin(a))}`)
    .join(' ');
...
function playerMark(player, isCarrier, tucked, showVelocity) {
  ...
  if (showVelocity) parts.push(velocityTriangle(player)); // last, so it draws over the body
  return (
