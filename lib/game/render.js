 ...
 */
import { VIEWBOX_WIDTH, PRESS_BOX_X, CENTRE_X, SIDELINE_LEFT, SIDELINE_RIGHT, UNITS_PER_YARD_X, y as yardToY } from '../field/geometry.js';
...
import { gameView, GOAL_YARD, END_YARD, fieldPos } from './view.js';
...
/**
 * The two quick-press buttons, stacked in the same margin column as the
 * COACHES MENU label — one above it, one below. They are for the two things
 * a coach does over and over (shuffle the formation, run the turn), so they
 * sit on the board itself rather than behind a menu that has to be opened
 * first. Square plates, sized to a thumb like the menu hit area is, and set
 * clear of that area so neither is pressed by mistake.
 */
const FIELD_BTN_SIZE = 9;
const FIELD_BTN_GAP = 6;
/**
 * The buttons' centre line. NOT PRESS_BOX_X: the label is rotated a quarter
 * turn about its own baseline, so the column its glyphs actually occupy sits
 * to the RIGHT of that baseline — from the baseline less the descent to the
 * baseline plus the ascent. Measured in the browser at the .pb size (8.5px):
 * the label runs x 254.9 to 265.0 about a baseline of 257, so its column is
 * centred a shade under three units right of it.
 *
 * Centring the buttons on the baseline instead put their left edge barely a
 * unit from the right-hand yard numbers, which end at x 251.2 — close enough
 * to read as part of the field rather than as controls. On the label's column
 * they are the same width as the text they sit under (9 against 10.04) and
 * clear the numbers by four.
 */
const LABEL_COLUMN_CENTRE = PRESS_BOX_X + 2.94;
...
export function renderBoardShell(losYard) {
  ...
  return {
    ...
    markup:
      ...
      `<g id="game-menu">${menuButtonMark()}</g><g id="game-buttons"></g>` +
      `<g id="game-message"></g>`,
  };
}
...
function fieldButtonMark({ attr, icon, label, cy, on = false, off = false, pressed = null }) {
  ...
  return (
    ...
    ` x="${num(LABEL_COLUMN_CENTRE - FIELD_BTN_SIZE / 2)}" y="${num(cy - FIELD_BTN_SIZE / 2)}"` +
    ` width="${num(FIELD_BTN_SIZE)}" height="${num(FIELD_BTN_SIZE)}" rx="${num(FIELD_BTN_RADIUS)}"/>` +
    `<text class="fbtn-icon" x="${num(LABEL_COLUMN_CENTRE)}" y="${num(cy)}">${escapeText(icon)}</text>` +
    `</g>`
  );
}
...
export function renderFieldButtons(state, { repositioning = false, animating = false } = {}) {
  const view = gameView(0);
  const midY = (view.fieldTopY + yardToY(view, END_YARD)) / 2;
  // Clear of the menu hit area, which straddles the label between them.
  const offset = MENU_HIT_HALF_H + FIELD_BTN_GAP + FIELD_BTN_SIZE / 2;
  const parts = [];
