 ...
 * velocity, out to radius + speed × DEBUG_VELOCITY_SECONDS — the same reach
 * the old hairline drew, so the apex lands exactly on the old tip. The radius
 * term is what puts the apex outside his own circle at any speed that rounds
 * to a visible difference; the length BEYOND his edge is the part that reads
 * as velocity, and it is strictly proportional to it. The other two vertices
 * trail the apex by ±120°, at the same distance from the centre, which is
 * what makes the circumcentre — and so the triangle's visual centre — sit
 * exactly at the origin regardless of heading. The `speed === 0` guard exists
 * only to dodge dividing by zero when computing the heading (which would draw
 * NaN), not to catch "slow": standing still there is no direction to point,
...
function velocityTriangle(player) {
  ...
  if (speed === 0) return '';
  const r = player.radius + speed * DEBUG_VELOCITY_SECONDS;
