...
export function checkIncomplete(state, { endOfTurn = false } = {}) {
  ...
  if (!endOfTurn && len(state.ball.vel) > PASS_DEAD_SPEED) return [];
...
export function checkDeadBall(state) {
  ...
  if (!car && (bp.x < SIDELINE_LEFT || bp.x > SIDELINE_RIGHT)) {
    state.deadReason = 'out-of-bounds';
    return [{ type: 'out-of-bounds' }];
  }
  return [];
 ...
 * push the formation out of frame. Two cases ignore where the ball stopped:
 * an enforced flag comes back from the previous line, and so does an
 * incomplete pass, which by rule gains nothing.
 */
