 ...
 *
 * An offensive lineman is an ineligible receiver on a forward pass — the ball
 * sails through him rather than sticking, same as if he were not standing
 * there at all. `state.ball.forward` is only ever set by releasePass, so a
 * fumble (dropBall) or a handoff/lateral (a backward throw) never trips this:
 * he can still scoop either of those up like anyone else.
 */
export function checkPickup(state) {
  ...
  for (const p of state.players) {
    ...
    if (dist(p.pos, state.ball.pos) <= p.radius + PICKUP_RADIUS_BONUS) {
      // `atYard` is the catcher's own spot, taken before state.ball is
      // cleared out from under him -- it is what pairs this pickup back to a
      // pass event's `fromYard` to measure air distance exactly.
      const atYard = yardsOfY(p.pos.y);
      state.ball = { carrierId: p.id, pos: null, vel: null };
      if (p.team === 'defense') state.deadReason = 'recovered';
      return [{
        type: 'pickup', by: p.id, team: p.team, atYard,
      }];
    }
  }
  return [];
