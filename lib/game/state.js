...
export function createGame({
  ...
  const state = {
    seed,
    aiTeam: ai,
    // Training mode's "computer plays both sides" — orthogonal to aiTeam
    // (which stays null, hot-seat, whenever this is on) rather than a third
    // value for it, because aiTeam is read all over the codebase as "the
    // team that is NOT the human's to plan", and there is no such team here:
    // every man on the board is still legal for a human hand to drag, the
    // computer is just also drawing up both sides of it. See ai.js's
    // AI_MODES and app/main.js's pressAi/pressRun.
    aiBoth: false,
    ...
    // {look, snapped, call} — see lib/game/read.js, which builds it and
    // advances it, though it is not the only writer: the learned defense's
    // committedScheme/committedCover (lib/game/learned/defense-policy.js)
    // write playRead.call.defense once the down commits to a scheme and a
    // coverage map. Per-down like plannedPass, and cleared by nextDown for
    // the same reason.
    playRead: null,
...
export function setMode(state, id, mode) {
  ...
  // Untucking drops the ball's protective wrap — the one stance a throw can
  // still be planned underneath, since setPass itself does not gate on mode.
  // A throw planned while tucked cannot survive the wrap coming off, so it is
  // cancelled here rather than left to be thrown by a runner who no longer
  // has the grip for it.
  if (p.mode === 'tucked' && mode === 'normal' && state.plannedPass?.from === id) {
    state.plannedPass = null;
  }
  p.mode = mode;
