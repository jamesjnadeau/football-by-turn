...
function makePlayer(spec, team, losYard) {
  return {
    ...
    cover: null,
    // Yards of leverage, in SVG units, that a cover order is being played with:
    // signed along y, added to coverAim's answer. Zero for an ordinary cover
    // order; non-zero only when the learned defense's read has committed and it
    // is playing this man from downhill or from depth. Cleared with the order.
    coverShade: 0,
    tackleCooldown: 0,
  };
}
...
export function setPlan(state, id, dir, throttle, target = null, short = false) {
  ...
  p.coverShade = 0; // the shade belongs to the cover order, not to him
}
