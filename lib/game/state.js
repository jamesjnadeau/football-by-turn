 ...
 */
import { fieldPos, TOP_YARD, END_YARD } from './view.js';
...
/**
 * Whether anyone may be repositioned right now. Setting up is something you do
 * at the line, so it is over the moment the first turn runs. app/main.js asks
 * this too — it decides whether the board's reposition button is shown at all
 * — which is why the rule is named here rather than spelled out twice.
 */
 ...
 *
 * Also bounded by the field itself. The LOS rule alone would happily set a
 * receiver down in the stands, which never came up while nothing called this
 * — the whole player has to fit inside the sidelines and between the top of
 * the frame and the end line, so `radius` is taken off each edge rather than
 * checking his centre.
 */
export function placePlayer(state, id, pos) {
  ...
  if (pos.x - p.radius < SIDELINE_LEFT || pos.x + p.radius > SIDELINE_RIGHT) return false;
  if (pos.y - p.radius < fieldPos(0, TOP_YARD).y) return false;
  if (pos.y + p.radius > fieldPos(0, END_YARD).y) return false;
  p.pos = pos;
  return true;
}
