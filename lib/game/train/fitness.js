...
// What a perfectly-read down is worth, in yards. Measured at 2: as-trained,
// trigger-disabled and never-commits all scored identically to three
// decimals — the read's weights have no measurable effect on yards allowed,
// so this term and the yardage terms above steer disjoint parts of the
// genome. Pricing accuracy highly cannot trade away defensive quality
// because there is nothing to trade it against; it only has to outweigh the
// noise the read's own consumer (the coverage shade) injects into those
// yardage terms, which the measurement above put at roughly 7 yards between
// otherwise-identical evaluations. At 2, the whole climb from
// always-guessing-run to a perfect read was worth 1.79 yards — smaller than
// that noise, so the accuracy signal never surfaced. 20 makes that same
// climb worth almost 18 yards, enough to dominate the noise instead of
// getting lost in it. The number is a training signal sized to be heard
// over that noise, not a claim that a correct read is worth twenty yards of
// football.
export const READ_ACCURACY_YARDS = 20;
 ...
 *
 * The read-accuracy term stands apart from all of those: it does not split on
 * touchdown/non-touchdown, and it is added once, flat, rather than folded into
 * the per-play rule above, because it is not a play outcome at all — it is
 * `stats.readAccuracy`, harness.js's summarizePlays' own ratio of right reads
 * over every read scored in the whole batch. `null` there (nothing was ever
 * dealt, so nothing was ever scored — scriptedOffenseCoach, co-evolution's
 * learned offense, and every train:vs-ghost run) has to make this term
 * contribute exactly 0, or a caller that has never heard of dealtOffenseCoach
 * would see its score move the moment this term was added.
 */
export function defenseFitness(stats) {
  return -stats.gainYardsPerPlay
    ...
    + (stats.readAccuracy == null ? 0 : READ_ACCURACY_YARDS * (2 * stats.readAccuracy - 1));
}
