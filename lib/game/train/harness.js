...
/**
 * What the dealt arm says a covering defender's read SHOULD have answered,
 * this turn. `forwardPasses` is `state.forwardPasses` as the coaches saw it —
 * before this turn's runTurn can change it — which is what makes the
 * play-action arm's truth flip mid-down: 'run' while the fake is still
 * selling, 'pass' the instant a real throw has gone up. That flip is not a
 * wrinkle to work around, it is the whole mechanic dealtOffenseCoach's third
 * arm exists to teach — reading run during the fake is correct, and reading
 * pass once the ball is up is also correct.
 */
export function dealtTruth(dealtArm, forwardPasses) {
  if (dealtArm === 'play-action') return forwardPasses > 0 ? 'pass' : 'run';
  return dealtArm === 'pass' ? 'pass' : 'run';
}
 ...
 *
 * readsRight/readsTotal score every covering defender's read against the
 * dealt arm's ground truth (see dealtTruth), turn by turn, after the coaches
 * have run and before runTurn moves the ball — the instant this play's
 * evidence for that turn is exactly what the coaches themselves saw. Left at
 * zero (never scored) whenever nothing was dealt (`state.dealtArm` unset,
 * true of scriptedOffenseCoach and of co-evolution's learned offense) or
 * nobody has a man to read yet (`state.playRead.reads` empty, true of every
 * zone down and of turn 0 before any cover map exists). Kept as raw counts
 * rather than a per-play ratio because summarizePlays has to sum them across
 * plays of different lengths before dividing.
 */
export function playOnePlay(state, offenseCoach, defenseCoach, random) {
  ...
  for (; turns < MAX_TURNS_PER_PLAY && state.phase !== 'playOver'; turns++) {
    offenseCoach(state);
    defenseCoach(state);
    if (state.dealtArm && state.playRead?.reads) {
      const reads = Object.values(state.playRead.reads);
      if (reads.length > 0) {
        const truth = dealtTruth(state.dealtArm, state.forwardPasses);
        for (const read of reads) {
          readsTotal += 1;
          if ((read.pass > 0 ? 'pass' : 'run') === truth) readsRight += 1;
        }
      }
    }
...
export function defenseCoach(values) {
  return (state) => {
    ...
    // The read this turn, not last turn's. runTurn advances the percept and
    // THEN coaches; playOnePlay coaches and then calls runTurn, so without
    // this the orders below would answer a read taken from the field two
    // turns ago — and every weight would be selected under a lag the real
    // game does not have. advancePlay is idempotent per turn, so runTurn's
    // own call becomes a no-op.
    advancePlay(state, values);
 ...
 * examples of a live throw on a play that is still, unambiguously, a run.
 * The third arm is written, because no coach recorded a fake.
 *
 ...
 * the same way.
 *
 * readAccuracy sums playOnePlay's readsRight/readsTotal counts across every
 * play (touchdown or not — a read is scored the instant it is made, before
 * the down's outcome is known) and divides once at the end, which is what
 * lets it stay a single ratio over plays of different lengths instead of a
 * mean of per-play ratios. `null` when no reads were scored anywhere in the
 * batch — every result missing readsRight/readsTotal entirely (hand-built
 * play objects, as several tests in this file use) or every result's
 * readsTotal at zero (scriptedOffenseCoach and co-evolution, where nothing is
 * ever dealt) both land here, and fitness.js's defenseFitness has to treat
 * that `null` as contributing nothing, or every caller that never deals an
 * arm would see its score move.
 */
export function summarizePlays(results) {
  ...
  for (const r of results) {
    ...
    totals.readsRight += r.readsRight ?? 0;
    totals.readsTotal += r.readsTotal ?? 0;
  }
  return {
    ...
    readAccuracy: totals.readsTotal > 0 ? totals.readsRight / totals.readsTotal : null,
  };
}
