...
export function defenseCoach(values) {
  return (state) => {
    ...
    // Nothing on this path reads it back: advancePlay (below) builds no
    // genome-dependent percept any more, and this function hands `values`
    // straight to learnedOrders itself, not through activeGenome. Kept
    // anyway, as insurance — it is the channel active.js documents for a
    // candidate genome reaching the engine as plain state, and losing it
    // silently is exactly the failure that already cost this branch a cycle
    // once (a candidate scored on the shipped genome instead of its own).
    state.genomeOverrides = { ...state.genomeOverrides, defense: values };
    // This turn's percept, not last turn's. runTurn advances it and THEN
    // coaches; playOnePlay coaches and then calls runTurn, so without this
    // the orders below would answer a look taken from the field two turns
    // ago. advancePlay is idempotent for the whole down, so runTurn's own
 ...
 * does not happen to have recorded: neither logged offense ever sells a run
 * and then throws, so without a scripted arm a defense trained against this
 * distribution would never once see misdirection, and a defense that has
 * never seen a fake is not being trained against one. Turn 0 is
 * planLearnedRun's own picture, so the run keys are the real ones: the line
 * drives, the back leans, the quarterback carries out a fake. Turn 2 is the
 * throw.
 *
 ...
 * turn: a play that changed its mind at turn two — a run arm suddenly
 * throwing, or a pass arm suddenly running — would not be any of the three
 * arms above. It would be incoherent football, and there is no defense worth
 * training against a play that cannot exist.
 */
