 ...
 */
import { stepPhysics, contactsAt } from './physics.js';
 ...
 */
function snapshot(state, contacts = []) {
  ...
  return {
    ...
    frictions: frictionsOf(contacts),
  };
}

/**
 * The friction the board draws: one entry per MAN per point of friction, each
 * carrying the bearing from him toward the opponent leaning on him. Two men
 * hand-fighting produce two entries, and a man squeezed from both sides
 * produces one per squeezer -- which is what the renderer wants, since it
 * draws an arc on a man in a direction and has no interest in who the pair
 * were.
 *
 * Only opponents count. Teammates bump into each other constantly on a
 * crowded field, and marking every one of those would paint the whole line
 * orange and say nothing.
 */
function frictionsOf(contacts) {
  const out = [];
  for (const c of contacts) {
    if (c.a.team === c.b.team) continue;
    // The normal runs a -> b, so it is already the bearing from a toward b;
    // b's own bearing is the same line walked the other way.
    out.push({ id: c.a.id, angle: Math.atan2(c.normal.y, c.normal.x) });
    out.push({ id: c.b.id, angle: Math.atan2(-c.normal.y, -c.normal.x) });
  }
  return out;
}

/**
 * The friction on the board as it stands, in the same shape a frame carries:
 * one entry per man per point of friction, each with the bearing toward
 * whoever is leaning on him.
 *
 * Same rule as the animated case, because it is the same function doing the
 * judging -- teammates bumping do not count, only opponents.
 */
export function frictionsAt(state) {
  return frictionsOf(contactsAt(state));
}

/**
 * Who still needs an arrow — this coach's warning feed, so it skips every man
 * who is not his to order: the team the computer coaches, and in a match the
 * team the other coach does. The computer's are planned inside runTurn, after
 * this has had its say; the remote coach's arrive with his commit.
 */
export function unplannedPlayers(state) {
  return state.players
    .filter((p) => p.plan === null && p.team !== state.aiTeam && p.team !== state.remoteTeam
...
export function runTurn(state, random) {
  ...
  for (let i = 0; i < SUBSTEPS_PER_TURN; i++) {
    ...
    const contacts = stepPhysics(state, DT);
    ...
    frames.push(snapshot(state, contacts));
